...
begin
  ...
  insert into wayv_members(group_id,user_id,nickname) values(target.id,auth.uid(),trim(nickname)) on conflict(group_id,user_id) do update set nickname=excluded.nickname,status='pending' returning * into member;
